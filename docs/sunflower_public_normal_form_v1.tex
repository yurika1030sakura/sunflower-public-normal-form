...
\title{Coordinate Collision and Amortized Accounting in Sunflower Bounds}
\author{
Yuli Li\\
...
code after the public value pins already present in $\mathcal P$.  The law
$\mu$ need not be uniform away from the root: in nonroot nodes it is the public
one-sample marginal, or the corresponding public tuple marginal in tuple
modes, obtained from the root samples by the paid conditioning, pruning, and
quotient operations on the branch.  It is always supported on the current
public cylinder of $\C$.  Heavy coordinate fibres are immediate exits of the
proof tree, not additional entry assumptions.
\end{definition}
...
recorded.  For the current persistent signature the public state carries an
old-pattern domain $P\subseteq U$ on which the five-state equality pattern has
already been paid.  Later old-pattern exposures with the same signature only
reveal the pattern on $O\setminus P$ and then replace $P$ by $P\cup O$.
Changing the active signature is not a free relabelling.  Before a different
signature may be used on old-supported data, the current old window is either
ended by a terminal, fresh, hash, scale, or old-incidence exit, or it is
coarsened by the quotient-erasure rule of
Theorem~\ref{hyp:witness-reset}.  In that quotient step the old pattern and
the fresh witness labels are analysis data: they are not retained in the
terminal sigma-algebra, and the continuing state keeps only the old support,
the dyadic scale, the active public law, and the capped revisit counter vector.
Pattern-support charges are not old close-pair incidences and do not enter the
revisit counters;
...
old-incidence ledger or erased by the quotient rule.
...
Old-pattern support exposures and old close-pair incidences are separate
accounts.  Lemma~\ref{lem:old-residual-routing} may expose the five-state
pattern on a deterministic possible support $O$, but this only enlarges the
pattern domain $P$ for the current persistent signature and debits the pattern-support
account.  If the active witness signature changes before the window is closed,
the branch is routed through the quotient-erasure rule of
Theorem~\ref{hyp:witness-reset} rather than inheriting the pattern account for
free.  The raw revisit counters are incremented only when
an actual split old close-pair block has been determined on the current atom.  Those actual
...
these are precisely the old-window continuations.  Each such registration
increments the raw revisit counter and debits the weighted old account, while
the high-revisit exit stops the low-revisit regime when the raw counter reaches
the cap on a coordinate which is split in the current close-pair block.
...
\begin{proposition}[Quotient transcript entropy ledger]
\label{prop:quotient-tree-entropy-ledger}
The conclusion of Proposition~\ref{prop:tree-entropy-ledger} remains valid if
some internal steps expose a private analysis selector $A_v$ but immediately
replace it, before any descendant terminal choice is made, by a retained
quotient selector
\[
  \overline A_v=q_v(A_v,\mathcal F_v)
\]
and the entire descendant subtree below that step is measurable with respect to
the public sigma-algebra generated by the previous retained selectors and
$\overline A_v$.  In that case the entropy contribution of $v$ is
\[
  \bfH(\overline A_v\mid E_v),
\]
not $\bfH(A_v\mid E_v)$.  Equivalently, if nonnegative local charges satisfy
\[
  \bfH(\overline A_v\mid E_v)\le \EE[c_v\mid E_v]
\]
for quotient steps, and the usual selector inequality for nonquotient steps,
then
\[
  \bfH(\Theta)\le \EE\sum_{v\ {\rm reached}}c_v .
\]
\end{proposition}

\begin{proof}
Form the retained terminal transcript $\overline\Gamma$ by scanning the proof
tree in topological order and recording, at each reached node, the ordinary
child selector for a nonquotient step and the retained quotient selector
$\overline A_v$ for a quotient step.  The hypothesis says that after a quotient
step no descendant selector and no terminal shadow depends on the discarded
part of $A_v$ except through $\overline A_v$.  Hence the projected terminal
shadow $\Theta$ is a function of $\overline\Gamma$.  The same chain-rule proof
as in Proposition~\ref{prop:tree-entropy-ledger}, applied to
$\overline\Gamma$, gives
\[
  \bfH(\Theta)\le \bfH(\overline\Gamma)
  =
  \sum_v \Prb(E_v)\bfH(\overline A_v\mid E_v)
\]
with ordinary selectors read as $\overline A_v=A_v$ at nonquotient nodes.
The charge bound follows by summing the local inequalities.
\end{proof}
...
is contained in $U$, first route it through the paid old-pattern support
account and then register only the actual split close-pair block as old
incidence; repetition above the revisit threshold triggers the high-revisit
exit only on such an actual split coordinate, while below that threshold the
incidences are debited in $N_{\rm old}$.  If a surviving hash block
...
fresh part is promoted, or if $B\subseteq U$ its old-pattern data are paid and
its actual split old incidences are registered unless high revisit triggers on
one of those split coordinates.  That classification is a terminal exit from the
...
pattern data and charges it to the old-pattern support account, not to the
old close-pair revisit counters.  On each resulting atom the old split set
$K=W_L$ is known inside the already paid old pattern; only the actual split
block extracted from $K$ is registered as old incidence.  Proposition~\ref{prop:sparse-residual-transition}
...
\[
  O=\{u\in U:\Prb_\nu[u\in W_L]>0\}.
\]
Then $O$ is public.  Before any nonterminal continuation, the five-state
equality pattern on $O$ is exposed through the old-pattern support ledger for
the current sample-label signature.  This exposure is not an old close-pair
incidence and does not increment the revisit counters.  On every resulting
positive-mass atom, the atom determines the actual old split set
$K=W_L\subseteq O$, which is nonempty, and produces an old close-pair block
$K'\subseteq K$ with
\[
  |K'|\ge |K|/3
\]
after relabelling.  Only the coordinates of this actual split close-pair block
are then offered to the old-window incidence ledger.  If registering $K'$
would exceed the revisit cap, the crossing coordinate lies in $K'$ and hence
has a genuine close-pair role on the current atom, so the high-revisit routing
of Lemma~\ref{lem:high-revisit} applies.  Otherwise the incidences of $K'$ are
debited in $N_{\rm old}$.  The pattern cost on $O$ is paid separately by the
old-pattern support account in $\Phi_{\rm old}$.
...
\begin{proof}
The set $O$ is determined by the current public law.  Let $\ell$ be the current
persistent ordered sample-label signature and let $P\subseteq U$ be the
old-pattern domain already paid for that signature in the current window.
Expose the five-state pattern only on the new part $O\setminus P$,
\[
  XYZ,\quad XY|Z,\quad XZ|Y,\quad YZ|X,\quad X|Y|Z
\]
and then replace $P$ by $P\cup O$.  This is a public refinement of
the old-pattern atom for $\ell$.  Its entropy is charged to the old-pattern
support account; no revisit counter is changed at this stage.

Since $W_L\subseteq O$ and $W_L$ is nonempty pointwise, the split coordinates
on any positive-mass atom are exactly a nonempty set $K=W_L\subseteq O$.
Among the split coordinates in $K$, one of the three roles
$XY|Z,XZ|Y,YZ|X$ occurs on at least one third; relabelling the samples gives
the close-pair block $K'$.  Now, and only now, the proof attempts to register
the incidences of $K'$.  If registering $K'$ would make some counter exceed
the revisit threshold, that coordinate belongs to $K'$ and the exposed role
gives a genuine close-pair revisit of it; the high-revisit exit is then the
exhaustive terminal/pruning/hash routing of Lemma~\ref{lem:high-revisit}.
Otherwise these actual split incidences are debited in $N_{\rm old}$, so they
lower $\Phi_{\rm old}$ in Section~\ref{sec:potential}.
...
\begin{theorem}[Refreshed old-window quotient erasure]
\label{hyp:witness-reset}
Consider an old-supported window for a fixed old support $U$, dyadic scale,
and persistent sample-label signature $\ell$.  Suppose a nonterminal
continuation attempts to replace $\ell$ by fresh witness labels before the
current old incidences have been registered or terminalized.  Then the proof is
put in normal form as follows.  Either the branch takes one of the charged
exits
\begin{enumerate}[label=\textup{(\roman*)}]
\item a terminal multi-sample projected shadow;
\item a fresh promotion, hash, product-KL, or deletion-trapped scale exit
already covered by the corresponding local ledger;
\item a persistent-reservoir return: after discarding a bounded amount of
auxiliary data, all remaining old split incidences in the window are witnessed
by one bounded reservoir, so Theorem~\ref{hyp:old-window} applies to actual
split blocks for that fixed signature;
\item an exponentially small witness-reset branch whose probability loss
exceeds its auxiliary non-value transcript entropy and is therefore charged as
a pruning-normalization loss.
\end{enumerate}
or the old-window transcript is quotiented before the new signature is used.
In the quotient branch the proof forgets the old pattern, the fresh witness
labels, and the order in which the old blocks were encountered.  The continuing
public state retains only
\[
  U,\qquad \text{the dyadic old scale},\qquad
  M=(M(i))_{i\in U}\in\{0,\ldots,Q\}^U,
\]
together with the terminal pins and hard-exit tokens already paid outside the
old window.  If an active child block, a queued old block, or a future terminal
coordinate selector depends on the old pattern or fresh witness labels being
erased, then that object must first be promoted to retained public data and
paid by the old-incidence, terminal-shadow, fresh, hash, or scale ledger.  It is
not part of the erased transcript.  All later selectors are required to be
measurable with respect to the conditional law on this coarsened event and
these retained variables.
Consequently the erased witness/pattern transcript cannot select a later small
terminal shadow.

Across a whole old-support epoch, repeated quotient resets overwrite the same
retained counter summary rather than appending a reset history.  The total
old information retained from all such resets is therefore bounded by the final
counter vector:
\[
  \bfH(M)\le |U|\log(Q+1),
\]
up to the fixed old-scale and queue labels.  Equivalently, if the proof ledger
charges the counter update locally, the increase of this summary entropy is
charged to the actual split incidences which changed $M$.  This is an
$O_Q(|U|)$ old-support charge over the whole epoch, not a charge paid once per
fresh signature.  The actual old close-pair incidences remain counted only
through the coordinates of the split blocks which were registered before the
quotient.  If one of those registrations would make $M(i)>Q$, then the crossing
coordinate is in the current actual split close-pair block, and the
high-revisit routing of Lemma~\ref{lem:high-revisit} applies before the
quotient is taken.
\end{theorem}

\begin{proof}
Let $\mathcal A$ be the finite or countable partition of the current node into
old-window analysis atoms: old-pattern atoms, fresh witness-label choices, role
vectors, and the order of the old blocks in the current window.  On each atom,
the proof either has already taken one of the charged exits listed above, or it
has merely registered actual split old incidences and has not exposed any old
symbol value.  In the latter case define the quotient map
\[
  a\in\mathcal A
  \longmapsto
  (U,\mathrm{scale},M(a),\text{already paid terminal/hard-exit data}).
\]
Replace all atoms with the same image by their union.  The child law is, by
definition, the conditional law on that union.  The later proof tree is then
run from this conditional law using only public tie-breaks and selectors
measurable with respect to the quotient state.  Before the replacement, every
block or coordinate selector which will still be used after the quotient is
either included in the image of the quotient map as paid retained data, or is
discarded together with the old-window analysis atom.  Hence the discarded
witness-label and old-pattern transcript is not part of the terminal
sigma-algebra and cannot choose a later coordinate shadow.

The only nonconstant old information retained by the quotient is the current
counter vector $M$.  Since each entry lies in $\{0,\ldots,Q\}$, the number of
possible vectors is at most $(Q+1)^{|U|}$, giving the displayed entropy bound.
If several quotient resets occur before the old support changes, the state does
not retain the list of earlier vectors or signatures; it retains only the
latest counter vector.  Thus the retained reset information over the epoch is
still bounded by the same counter-vector entropy.  Fixed scale labels, bounded
queues, and bounded reservoir labels contribute only a constant multiple of
$|U|$ under the chosen finite-state parameters.  Finally, the quotient is taken
only after all actual split incidences already seen have either been registered
or have triggered high revisit.  Thus a high-revisit coordinate is always in
the current actual split block, not merely in a possible support, so
Lemma~\ref{lem:high-revisit} is invoked with its genuine close-pair hypothesis.
\end{proof}
...
Let $O_1,\ldots,O_R\subseteq U$ be public actual split old close-pair blocks of
one active scale and one public sample-label signature $\ell$ for the ordered
active triple/reservoir labels, and let $N=\sum_r |O_r|$.  Deterministic
possible-support sets used only to expose old equality patterns have already
been charged to the old-pattern support account and are not members of this
list.  The window is run under the
normal-form rule of
...
inherited.  A continuing old-supported branch with fresh witness labels must
then be coarsened by Theorem~\ref{hyp:witness-reset}.
...
\item high revisit: adding the current actual split block would make some
$M(i)>Q$, and the current close-pair coordinate is routed by
Lemma~\ref{lem:high-revisit} into
...
\begin{proof}
The persistent signature $\ell$ is part of the public old-window state, so all equality
...
window is charged, it is treated as the hard exit which clears the bounded
queue, or else the old transcript is coarsened by
Theorem~\ref{hyp:witness-reset}; the old-pattern atom for $\ell$ is not reused
for the new labels.
...
$C_{\rm old}(Q,r_*)$ per registered actual split old incidence.  If adding the
incidences of $O_r$ would make some counter exceed $Q$, then, because $O_r$ is
an actual split old close-pair block, the current close-pair occurrence of
...
the weighted debit in $\Phi_{\rm old}=B_o|U|-N_{\rm old}-N_{\rm pat}$ pays it.
When these
...
$N_{\rm old}+N_{\rm pat}
\le (Q\omega_{\rm old}+C_{\rm pat}(r_*))|U|<B_o|U|$
in the constant hierarchy.
\end{proof}
...
Fix an old support $U$, a dyadic old scale, and a persistent public
sample-label signature $\ell$ for the ordered active triple/reservoir labels.
The proof maintains one
...
local charge and cleared; it is not inherited by the new signature.  A
continuing old-supported branch with fresh witness labels must then pass
through the quotient-erasure rule of Theorem~\ref{hyp:witness-reset}; a
different signature is not allowed to expose a new full old-pattern atom for
free.  A bounded
...
fresh promotion or to the old-pattern support account before any old incidence
...
For the current persistent old-window signature, let $P\subseteq U$ be the
old-pattern domain on which the five-state equality pattern has already been
paid.  Let $N_{\rm pat}$ be the weighted cumulative old-pattern support charge:
when a pattern is first exposed on $s$ new old coordinates for this persistent
signature, $N_{\rm pat}$ increases by the corresponding finite cost.  A branch
which attempts to replace the signature before closing the old window is not
allowed to start a second independent pattern account.  It must first either
take a charged hard exit or apply the quotient-erasure operation of
Theorem~\ref{hyp:witness-reset}, which clears the live pattern transcript and
retains only the capped counter vector.  Therefore
\[
  N_{\rm pat}\le C_{\rm pat}(r_*)|U|
\]
for the live signature on every branch.  This account pays possible-support
pattern exposures and is not an incidence counter.

Let $I_{\rm old}$ be the raw cumulative actual-split old-window incidence count
already registered on the current branch, capped by the high-revisit threshold
through the coordinate counters $M(i)\le Q$.  These incidences are only actual split
close-pair coordinates after the relevant old-pattern atom has been fixed.
After the fixed-window parameters $q,h,Q,r_*$ are chosen, let $C_{\rm fr}(q)$,
$C_{\rm hash}$, $C_{\rm pat}(r_*)$, and $C_{\rm old}(Q,r_*)$ denote the finite
transcript constants specified in Section~\ref{sec:constant-hierarchy}, and put
\[
...
Choose constants
\[
  B_o>Q\,\omega_{\rm old}+C_{\rm pat}(r_*)+C_{\rm aux}+10,
  \qquad
...
\[
  ...
  \qquad
  \Phi_{\rm old}=B_o|U|-N_{\rm old}-N_{\rm pat},
  \qquad
...
transcript.
Exposing old-pattern data on a deterministic possible support decreases
$\Phi_{\rm old}$ through $N_{\rm pat}$ instead; it never increments
$I_{\rm old}$ and therefore cannot by itself trigger high revisit.
Promoting any
...
$N_{\rm old}+N_{\rm pat}
\le (Q\omega_{\rm old}+C_{\rm pat}(r_*))|U|<B_o|U|$
before a terminal or bounded-support exit occurs.
...
\item If an old-pattern step for one persistent sample-label signature exposes the
five-state equality pattern on $s\ge1$ old coordinates which were not already
in the corresponding pattern domain $P$, then the entropy of this
possible-support exposure is charged by increasing $N_{\rm pat}$.  This step
births no active child and does not alter $I_{\rm old}$ or the counters
$M(i)$.  Before the live signature is closed or quotiented, the total such
debit is bounded by $C_{\rm pat}(r_*)|U|$.
\item If an old step registers $n\ge1$ actual split old-coordinate incidences
and births an active child $J$ with $|J|\le n$, then
\[
  \omega_{\rm old}n
  \ge
...
Suppose a nonterminal old exit registers $n\ge1$ actual split old-coordinate
incidences and births a continuing active child $J$ with $|J|\le n$, allowing
$J=\varnothing$.
...
rank/scale deposits, high-revisit selector costs on genuinely split
coordinates, and one possible hash token are all paid by the same
registered-incidence debit.  Pattern exposure on a larger deterministic
possible support $O$ is not included here; it is paid by the separate
old-pattern support account.
\end{lemma}
...
$\Phi_{\rm old}=B_o|U|-N_{\rm old}-N_{\rm pat}$ while $U$ is fixed and
$N_{\rm pat}$ is unchanged.  When $U$ grows, the
...
$N_{\rm old}+N_{\rm pat}
\le (Q\omega_{\rm old}+C_{\rm pat}(r_*))|U|<B_o|U|$
keeps $\Phi_{\rm old}$ nonnegative on nonterminal low-revisit branches.
...
old-pattern, old-incidence, or scale components of $\Phi$.  The rank and scale terms decrease
...
$m_i\le Q$ and accumulated pattern-support charge at most $c_i$ with
$\sum_{i\in U}c_i=N_{\rm pat}\le C_{\rm pat}(r_*)|U|$, deleting it changes
\[
  \Phi_{\rm old}=B_o|U|-\omega_{\rm old}I_{\rm old}-N_{\rm pat}
\]
by
\[
  -B_o+\omega_{\rm old}m_i+c_i
  \le -B_o+Q\omega_{\rm old}+C_{\rm pat}(r_*)<0.
\]
...
most $C_{\rm old}(Q,r_*)n$ on a branch registering $n$ actual split old
incidences.  Possible-support five-state pattern exposure is bounded
separately by $N_{\rm pat}$.
\end{enumerate}
\end{lemma}
...
let $\Gamma_v$ be the retained child selector after all deterministic public
tie-breaks and all immediate quotient-erasure maps at $v$ have been applied.
Thus, at a quotient old-window reset, $\Gamma_v$ is the retained counter-vector
update and any paid retained child data, not the discarded old-pattern or fresh
witness transcript.  Let $\ell_v^{\rm pr}$ be the
...
fresh potential\\[1mm]
old pattern support &
five-state pattern on deterministic possible support $O$ &
$\le \Delta N_{\rm pat}$ and no revisit counter is changed &
old potential\\[1mm]
old residual/window &
sample-label signature, role vector, incidence data on actual split blocks; no
high-symbol values &
$\le \Delta N_{\rm old}$ after the fixed old debit is applied &
old potential\\[1mm]
old quotient reset &
retained counter-vector update $M$ and any already paid child data; discarded
witness/pattern transcript not retained &
$\le \Delta N_{\rm old}$ incrementally; over one old epoch the retained counter
summary has entropy $\le |U|\log(Q+1)$ &
old potential\\[1mm]
...
of the continuing old-window tree.  Five-state equality patterns on
deterministic possible supports are charged first to $N_{\rm pat}$ and do not
enter the revisit counters.  After the actual split block has been determined,
the remaining sample-label signature, role, scale, reservoir-label, and
incidence data are bounded by $C_{\rm old}(Q,r_*)$ per registered actual split
old incidence.  The weighted debit $\omega_{\rm old}$ also reserves any
old-born rank and scale balances and any single hash token assigned to the
nonempty old exit.  The delayed queue prevents the same bounded short window
for a fixed signature from being repeatedly recharged without increasing the
raw actual-split counter and hence $N_{\rm old}$; if the signature changes, the
bounded queue is charged to that hard exit before it is cleared.
If the signature changes through the quotient-erasure route of
Theorem~\ref{hyp:witness-reset}, the retained selector is only the updated
capped counter vector together with any child block or terminal data already
paid by the old, shadow, fresh, hash, or scale ledger.  The old-pattern atom,
fresh witness labels, role-order history, and any block not promoted to paid
retained data are discarded.  Proposition~\ref{prop:quotient-tree-entropy-ledger}
therefore charges the quotient step by the entropy of the retained counter
summary, not by the full erased analysis transcript.  Since
$M\in\{0,\ldots,Q\}^U$ over an old-support epoch, this retained entropy is at
most $|U|\log(Q+1)$, and the incremental updates are paid by the registered
actual-split incidences.
...
\begin{proof}
Let $\overline\Gamma$ be the retained terminal transcript of the finite
truncation: it records ordinary non-public child selectors, quotient selectors
at erasure steps, pruning losses, and the terminal projected shadow after
unpaid analysis-only data have been projected away.  Erased old-pattern and
fresh-witness analysis atoms are not part of $\overline\Gamma$ unless they have
first been promoted to paid retained data.  Propositions~\ref{prop:tree-entropy-ledger}
and \ref{prop:quotient-tree-entropy-ledger}
give the retained transcript entropy, and
...
Old residual possible-support patterns are paid by $N_{\rm pat}$; the actual
old close-pair blocks extracted from them are paid by weighted old incidence,
including the old-supported residual witness routing of
Lemma~\ref{lem:old-residual-routing}.  Pooled close-pair blocks are paid by the
incidence window which produced them.  High-revisit bounded-support blocks are
paid by the repeated actual split old incidences that triggered the
high-revisit exit.
...
witnesses are either actual split incidences paid by the old-incidence ledger
or analysis data erased by the quotient rule, rather than by exposing $W_L$ as
a terminal selector.
...
\[
  C_{\rm fr}(q),\qquad C_{\rm hash},\qquad
  C_{\rm pat}(r_*),\qquad C_{\rm old}(Q,r_*),
\]
...
Proposition~\ref{prop:hash-ledger}.  The pattern-support constant
$C_{\rm pat}(r_*)$ bounds all five-state equality-pattern exposure on
deterministic old possible supports for one persistent old-window signature.
A change of witness signature is handled by the quotient-erasure theorem,
Theorem~\ref{hyp:witness-reset}.  Across an old-support epoch, the only retained
nonconstant old information from such resets is the current capped counter vector
$M\in\{0,\ldots,Q\}^U$, whose entropy is at most $|U|\log(Q+1)$.  Equivalently,
for one live signature the cumulative weighted pattern-support charge satisfies
$N_{\rm pat}\le C_{\rm pat}(r_*)|U|$.  This charge is separate from close-pair
incidence and cannot trigger high revisit.  $C_{\rm old}(Q,r_*)$ is the
maximum old-window auxiliary transcript cost per actual split old incidence
before the revisit cap $Q$, including the sample-label signature, three-role
refinement, bounded reservoir labels, the bounded queue-flush charge, and the
incremental entropy of the quotient-reset counter summary.  It excludes
possible-support pattern
exposure, old-born rank/scale deposits, and a possible assigned hash token, which are
paid by the later choice of $\Phi_{\rm old}$ and $\omega_{\rm old}$.  All four
constants are finite because $q,h,Q,r_*$ and the window length are fixed
constants.
...
$C_{\rm fr}(q)$, $C_{\rm hash}$, $C_{\rm pat}(r_*)$, and
$C_{\rm old}(Q,r_*)$ defined above are finite.  Since $4\sigma<1$, choose
...
Proposition~\ref{prop:global-charge}: the stronger old inequality keeps
$\Phi_{\rm old}=B_o|U|-N_{\rm old}-N_{\rm pat}$ nonnegative before high revisit
because $N_{\rm old}=\omega_{\rm old}I_{\rm old}$, $I_{\rm old}\le Q|U|$, and
$N_{\rm pat}\le C_{\rm pat}(r_*)|U|$.  The pattern-support term pays
five-state pattern exposure on deterministic possible supports; it does not
enter the close-pair revisit counters.  The weighted debit $\omega_{\rm old}$
pays actual old-incidence transcript, reserves any old-born rank and scale
balances, and pays any single hash token assigned to that nonempty old exit,
while
...
\begin{aligned}
  ...
  C_{\rm rk},\ C_{\rm fr}(q),\ C_{\rm hash},\ C_{\rm pat}(r_*),
  C_{\rm old}(Q,r_*),\\
...
\begin{align*}
  ...
  B_o&=O(Q(C_{\rm old}+B_{\rm rk}+B_s+C_{\rm hash}+1)
    +C_{\rm pat}+C_{\rm aux}+1),\\
...
positive support by exposing old-coordinate pattern data only as live analysis
data for the current signature; actual split coordinates are registered in the
old-incidence ledger, and refreshed signatures are coarsened by
Theorem~\ref{hyp:witness-reset}.
