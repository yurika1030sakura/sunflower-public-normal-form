...
During one old-support epoch, suppose every sparse old-supported pattern is used only as an analysis partition, and that before any nonterminal child outside the epoch is entered every old coordinate block that can affect a future selector satisfies Lemma~\ref{lem:crit-sparse-no-selector}.  Conditional on the old support $U$, the entire internal old-pattern transcript of the epoch may be quotiented to a retained old output $S_{\old}^{\rm out}$ with
\[
        \HH(S_{\old}^{\rm out}\mid U)
        \le C_{\old}|U|+O(1),
\]
where $C_{\old}$ depends only on $Q$ and on the bounded queue/reservoir constants.
\end{lemma}

\begin{proof}
Let $\mathcal A$ be the full analysis transcript formed by all possible supports $O$, five-state equality patterns on those supports, and actual split subsets inspected inside the epoch.  These objects are not retained labels.  The retained transition rule maps $\mathcal A$ to the quotient output
\[
        S_{\old}^{\rm out}
        =(M,\hbox{bounded queues},\hbox{active old blocks},
          \hbox{finite role/scale labels},\hbox{paid hard-exit data}).
\]
After quotienting, the child law is the conditional law on the union of all analysis atoms with the same value of $S_{\old}^{\rm out}$, and all later selectors are recomputed from this quotient law and the retained output.  Hence the retained tree records $S_{\old}^{\rm out}$, not the path transcript $\mathcal A$.

The output $S_{\old}^{\rm out}$ contains the capped vector $M\in\{0,\ldots,Q\}^{U}$, bounded queue positions, bounded active old blocks, finite role labels, finite scale labels, and at most one bounded token.  Each coordinate of $U$ has only finitely many possible annotations, and the number of active positions is bounded.  Thus the number of possible retained old outputs is at most $\exp(C_{\old}|U|+O(1))$.  Taking logarithms gives the entropy bound.  No term of the form
\[
        \sum_{\ell}\log\binom{|O_\ell|}{|K_\ell|}
\]
is charged, because the individual sparse analysis atoms are erased before any future selector is allowed to depend on them.
\end{proof}
...
\text{old epoch quotient:} &\quad \HH(S_{\old}^{\rm out}\mid U) &&\le C_{\rm blk}|U|\le \tfrac12 B_o|U|,            &&\tag{T6}\label{eq:T6}\\
...
\begin{proof}
The inequalities \eqref{eq:T1}--\eqref{eq:T3} are definitions of the terminal and pruning accounts.  For \eqref{eq:T4}, fresh coordinates are promoted only once; after promotion they leave the fresh reserve, so the losses $B_f|F_v|$ are disjoint along a branch.  Inequality \eqref{eq:T5} charges only hard old data retained after quotienting; that data is supported on actual split-coordinate mark increments, so it is paid by $\omega_{\old}\sum_i\Delta M(i)$, and each coordinate increments at most $Q$ times.  Inequality \eqref{eq:T6} is the epoch-level quotient bound of Lemma~\ref{lem:crit-old-entropy}: sparse old equality patterns are replaced by $S_{\old}^{\rm out}$ instead of being charged as a per-pattern path transcript.  The constant hierarchy chooses $B_o>2C_{\rm blk}+2Q\omega_{\old}+10$, so the old reserve pays both retained finite-state entropy and the debit of all mark increments without charging the internal sparse path entropy.
