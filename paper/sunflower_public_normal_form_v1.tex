...
Proposition~\ref{prop:product-kl-transfer}, positive-KL outputs by
...
\begin{proof}
If the terminal atom alternative holds, stop.  Assume it does not.  In
particular,
\[
...
projected shadows; KL/collision exits give a terminal projected tuple pin or enter hash
mode; the law-level near-sunflower exit enters residual mode.
...
shadow, a high-revisit hash/value-pin exit, or a low-revisit continuation
whose non-value transcript has been registered in the old-incidence ledger.
...
block, hash-window nodes carry a sample-label window and no pending returned
hash token, residual nodes carry an exact split block returned by activation,
hash, or close-pair, and old-window nodes carry old blocks of one scale and one
sample-label signature.  The listed exit alternatives for the active mode form
an exhaustive partition after deterministic public tie-breaks.  Every child is
either terminal or again belongs to one of those modes.
\end{proposition}
...
only terminal low-entropy/atom exits, product-KL or collision exits routed by
Proposition~\ref{prop:product-kl-transfer} and
Lemma~\ref{lem:collision-exit-pin}, or a law-level near-sunflower residual
return; the theorem's decision order makes these exits a partition.  In
hash-window mode, the state is a single labelled window.  Coordinates for
another sample label are placed in that label's separate window and are not
merged into the current transcript.  Theorem~\ref{thm:public-hash} requires
coordinate production to be public for the fixed label of the window and pays
the public transcript before any copying, while
...
pruning from Lemma~\ref{lem:heavy-or-light-hash}, and the public fresh-reset
pruning of Lemma~\ref{lem:fresh-reset-pruning}.  Their total discarded mass is
...
\begin{lemma}[One-coordinate collision exits are tuple pins]
\label{lem:collision-exit-pin}
Let $p,q$ be one-coordinate laws.  If
\[
...
$\tau\le\min(\alpha,\gamma_0^2)$, the cross-collision and high-side-collision
exits in Theorem~\ref{thm:close-pair-exit} terminalize as projected tuple
pins.
\end{lemma}
...
high-side statement follows from $\Coll(p)\le\max_a p(a)$.  A coordinate atom
of conditional mass at least $\tau$ is terminalized by
Lemma~\ref{lem:tuple-pin-shadow}.
\end{proof}
...
coordinate is not live, the branch stops immediately as a terminal value-pin
shadow.  A coordinate which is live at the terminal transcript is called
surviving.
...
Definition~\ref{def:hash-survival}, the branch terminalizes.  Thus on a
...
then at least one of the following holds:
\begin{enumerate}[label=\textup{(\roman*)}]
\item some codeword $x$ has $\mu(x)\ge \rho/6$;
...
$\Delta\ge\rho/6$, then $\max_x\mu(x)\ge \Delta\ge\rho/6$, giving the heavy
atom branch.  Otherwise $3\Delta<\rho/2$, and the distinct lift has mass at
least $\rho/2$.
\end{proof}
...
\item terminal codeword atom:
  \[
    \mu(x)\ge \rho/6\quad\text{for some }x\in\C;
  \]
...
$t$ be the first index whose current conditional one-coordinate KL is positive,
and let $i$ be the first index, if any, for which the realized prefix
$V_{\le i}=u_{\le i}$ has
\[
...
\begin{proposition}[Product-KL excess has a one-coordinate public exit]
\label{prop:product-kl-transfer}
In every product-KL excess branch in Theorem~\ref{thm:close-pair-exit}, the
excess
\[
  D(P\|Q_E)>\epsilon
\]
can be routed, using only the current public tuple state and paid transcript,
to one of:
\begin{enumerate}[label=\textup{(\roman*)}]
\item a terminal projected tuple shadow whose local entropy cost is charged by
the proof-tree ledger;
\item a public low-collision hash coordinate for an actual sample coordinate,
not merely a low-collision law on the pair alphabet $(A_j,B_j)$.
\end{enumerate}
\end{proposition}
...
tree.  On every hash alternative, the selected local variable is now an actual
...
\[
  \Prb[A_i=a]\ge \tau
\]
for some symbol $a$, or
\[
  \Coll(A_i)=\sum_a\Prb[A_i=a]^2\le \tau.
\]
Thus a revisit of $i$ gives either a one-coordinate projected tuple pin or a
low-collision conditional hash coordinate.
\end{lemma}

\begin{proof}
If $\max_a\Prb[A_i=a]\ge\tau$, the first branch holds.  Otherwise
\[
  \sum_a\Prb[A_i=a]^2
  \le
  \max_a\Prb[A_i=a]\sum_a\Prb[A_i=a]
  <\tau.
\]
\end{proof}
...
high-revisit exit is then the tuple pin/hash dichotomy of
...
close-pair exit, such as Proposition~\ref{prop:product-kl-transfer}, exposes a
...
current close-pair coordinate is routed by Lemma~\ref{lem:high-revisit} to a
terminal value shadow or to a public low-collision hash coordinate;
...
yields either a terminal coordinate value pin or a public low-collision hash
coordinate, which is alternative (2).  The current block is then charged to
that exit and is not counted as a further low-revisit continuation.
...
three conditioned copies, but after the public copy tax is paid the auxiliary
  labels are analysis-only data and are not retained.  The next state keeps only the
triple, pair, or bounded reservoir needed for the next transition.
...
\begin{proof}
The terminal-shadow part is (i).  The numerical inequalities used in the
fresh, old, parent-rank, and scale paragraphs are exactly
Lemma~\ref{lem:step-account-inequalities}; the assertion that a hard exit pays
at most one fixed hash token is Proposition~\ref{prop:hash-token-matching},
with Lemma~\ref{lem:token-free-children} preventing an unpaid token from being
...
\begin{proof}
The following table gives the selector and the local entropy bound.  All
...
shadow\\[1mm]
collision exit &
one coordinate value atom &
$O(1)$ after the fixed threshold &
shadow\\[1mm]
product-KL exit &
...
terminal tuple pins, immediate value pins, low common-symbol exits, and the
cross/high-collision exits of Lemma~\ref{lem:collision-exit-pin} are terminal
projected shadows.  Their value entropy is exactly the entropy of the pinned
coordinates and the sample labels retained in the shadow, hence it is charged
to $c_v^{\rm sh}$ and contributes to $T(\Theta)+R(\Theta)$.

The product-KL row is Proposition~\ref{prop:product-kl-transfer}.  The prefix
...
tuple prefix is amortized and terminal.  Positive-tail and first-crossing
	...
	$-\log(1-x)\le2x$ in Lemmas~\ref{lem:positive-tail} and
...
split exposure.  If the block is returned to residual mode, this charge is
...
one-coordinate hash output rather than to a terminal value pin, the selector
has fixed cost $O(1)$.  This cost is charged to the same old-incidence/high-revisit
account which detects the cap; if the coordinate later participates in a hash
return, the window transcript and copy cost are separately paid by the single
hash token assigned to the next hard exit.  Thus queue increments before a
completed window are not hidden inside the final token.
...
Proposition~\ref{prop:product-kl-transfer}, and choose the constants as in
...
summation, and scale birth.  Proposition~\ref{prop:product-kl-transfer}
